...
function MiniCompass({ azimuth, connected, reachable }: { azimuth: number; connected: boolean; reachable: boolean }) {
  ...
  return (
    <svg width="28" height="28" viewBox="0 0 28 28" className="shrink-0">
      {/* Cercle de fond */}
      <circle cx="14" cy="14" r="12" fill="none" stroke="currentColor" strokeOpacity="0.15" strokeWidth="1" />
...
function BigCompass({ azimuth, connected, reachable }: { azimuth: number; connected: boolean; reachable: boolean }) {
  const needleColor = connected ? "#22d3ee" : reachable ? "#fbbf24" : "#6b7280";
  const rad = (azimuth - 90) * (Math.PI / 180);
  ...
  return (
    <svg width="120" height="120" viewBox="0 0 120 120">
      {/* Cercle extérieur */}
      <circle cx="60" cy="60" r="56" fill="none" stroke="currentColor" strokeOpacity="0.1" strokeWidth="1.5" />
      {/* Cercle intérieur */}
      <circle cx="60" cy="60" r="44" fill="none" stroke="currentColor" strokeOpacity="0.08" strokeWidth="1" />
...
export function RotorWidget() {
  const { azimuth, connected, reachable, goTo, stop, isPending } = useRotor();
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          title={`Rotor — ${connected ? "Connecté" : reachable ? "API joignable" : "Non joignable"} · Azimut : ${azimuth}°`}
        >
          <MiniCompass azimuth={azimuth} connected={connected} reachable={reachable} />
          <span className="tabular-nums">{azimuth}°</span>
          <span
            className={cn(
              "h-1.5 w-1.5 rounded-full",
              connected ? "bg-cyan-400 animate-pulse" : reachable ? "bg-amber-400" : "bg-muted-foreground/30"
            )}
          />
        </button>
      </PopoverTrigger>
      ...
      <PopoverContent className="w-72 p-4" align="end">
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <span className="font-mono text-sm font-bold text-foreground">Rotor</span>
            <div className="flex items-center gap-1.5">
              <span className={cn("h-2 w-2 rounded-full", connected ? "bg-cyan-400 animate-pulse" : reachable ? "bg-amber-400" : "bg-muted-foreground/30")} />
              <span className={cn("font-mono text-[10px]", connected ? "text-cyan-400" : reachable ? "text-amber-400" : "text-muted-foreground/50")}>
                {connected ? "Connecté" : reachable ? "API OK" : "Hors ligne"}
              </span>
            </div>
          </div>
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className={cn("transition-opacity", isPending && "opacity-60")}>
              <BigCompass azimuth={azimuth} connected={connected} reachable={reachable} />
            </div>
            {/* Azimut au centre */}
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="font-mono text-lg font-extrabold tabular-nums text-cyan-400 leading-none">{azimuth}°</span>
            </div>
          ...
          {/* Lien vers l'app rotor */}
          <a
            href="https://arcorotctrl-dkbitpc4.manus.space"
            target="_blank"
            rel="noopener noreferrer"
            className="block text-center font-mono text-[9px] text-muted-foreground/50 hover:text-primary transition-colors"
          >
            Ouvrir l'interface rotor ↗
          </a>
        </div>
      </PopoverContent>
    </Popover>
